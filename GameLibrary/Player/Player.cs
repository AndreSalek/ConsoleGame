...
namespace GameLibrary.Player
{
    public abstract class Player : IPlayerWithAttributes
    {
        ...
        public IPlayer Duel(IPlayer opponent)
        {
            IPlayer winner;
            int turn = 0;
        ...
        private void SimulateTurn(IPlayer player, IPlayer player1)
        {
            ...
            {
                var dmg = DealDamage(player.MinDamage, player.MaxDamage);
                player1.Health -= dmg;
            ...
            {
                var dmg = DealDamage(player1.MinDamage, player1.MaxDamage);
                player.Health -= dmg;
        ...
        private int DealDamage(int minValue, int maxValue)
        {
            return new Random().Next(minValue, maxValue);
        }

        protected abstract bool BlockOrDodge(IPlayer attacker);
